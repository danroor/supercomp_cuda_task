...
__global__ void step1(double *r, double *Ar, double *w_prev, 
    ...
    if (left != -1 && i == 1 && j >= 1 && j <= N - 1) {
    	...
    	dbuf_cuda[j] = r[linearIdx];
    } else if (up != -1 && j == N - 1 && i >= 1 && i <= M - 1) {
    	ubuf_cuda[j] = r[linearIdx];
    }
...
__global__ void step3(double *r, double *w_prev, int M, int N, double tau,
    ...
    if (i > 0 && i < M && j > 0 && j < N) {
	    ...
	    if (left != -1 && i == 1 && j >= 1 && j <= N - 1) {
	    	...
	    	dbuf_cuda[j] = w_prev[linearIdx];
	    } else if (up != -1 && j == N - 1 && i >= 1 && i <= M - 1) {
	    	ubuf_cuda[j] = w_prev[linearIdx];
	    }
...
__global__ void fill_borders(double *matr, double *lbuf_cuda, double *rbuf_cuda,  double *ubuf_cuda, double *dbuf_cuda,
	...
	// threadIdx.x - номер элемента буферного массива
	// linearIdx  -линейный индекс граничного элемента в матрице
	int linearIdx;

	// проверка на случай N > M
	if (threadIdx.x <= M - 1) {
		// matr[threadIdx.x][0] -> get lin idx
		// linearIdx = 0 * (M + 1) + threadIdx.x = threadIdx.x
		if (down != -1) { matr[threadIdx.x] = dbuf_cuda[threadIdx.x]; }

		// matr[threadIdx.x][N] -> get lin idx
		linearIdx = N * (M + 1) + threadIdx.x;

		if (up   != -1) { matr[linearIdx] = ubuf_cuda[threadIdx.x]; }	
	}

	// проверка на случай M > N
	if (threadIdx.x <= N - 1) {
		// matr[0][threadIdx.x] -> get lin idx
		// linearIdx = threadIdx.x * (M + 1) + 0
		linearIdx = threadIdx.x * (M + 1);
		if (left  != -1) { matr[linearIdx] = lbuf_cuda[threadIdx.x]; }

		// matr[M][threadIdx.x] -> get lin idx
		linearIdx = threadIdx.x * (M + 1) + M;
		if (right != -1) { matr[linearIdx] = rbuf_cuda[threadIdx.x]; }
	}
}
...
void print_net_function(double *net, int M, int N,
	...
	for (int i = 0; i <= M; ++i) {
		printf("%10.5f ", A1 + i * h1);
	}

	printf("\n");

	for (int j = 0; j <= N; ++j) {
		printf("%10.5f ", A2 + j * h2);
		for (int i = 0; i <= M; ++i) {
			printf("%10.5f ", net[j * (M + 1) + i]);
		}
		printf("\n");
	}	
}
...
int main(int argc, char *argv[])
{
	...
	assert(threads_per_block > maxMN);
	...
	do {
		...
		fill_borders <<< 1, maxMN >>> (r, lbuf_cuda, rbuf_cuda,  ubuf_cuda, dbuf_cuda, M, N, left, right, up, down);
		...
		fill_borders <<< 1, maxMN >>> (w_prev, lbuf_cuda, rbuf_cuda,  ubuf_cuda, dbuf_cuda, M, N, left, right, up, down);
	...
	if (print_result && left == -1 && up == -1) {
		...
		// в левых границах переходим в глобальные координаты
